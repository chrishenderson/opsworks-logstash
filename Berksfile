# example cookbook 'redis', github: 'coderanger/chef-redis', tag: '1.0.4'
cookbook 'yum', '< 3.0.0'
#[submodule "elasticsearch"]
cookbook 'elasticsearch', 'github: chrishenderson/cookbook-elasticsearch'
#[submodule "kibana"]
cookbook 'kibana', 'github: foxycoder/chef-kibana'
#[submodule "logstash"]
cookbook 'logstash', 'github: foxycoder/chef-logstash'
#[submodule "redisio"]
cookbook 'redisio', 'github: brianbianco/redisio'
#[submodule "rabbitmq"]
cookbook 'rabbitmq', 'github: opscode-cookbooks/rabbitmq'
#[submodule "java"]
cookbook 'java', 'github: opscode-cookbooks/java'
#[submodule "apt"]
cookbook 'apt', 'github: opscode-cookbooks/apt'
#[submodule "erlang"]
cookbook 'erlang', 'github: chrishenderson/erlang'
#[submodule "mysql"]
cookbook 'mysql', 'github: opscode-cookbooks/mysql'
#[submodule "nagios"]
cookbook 'nagios', 'github: opscode-cookbooks/nagios'
#[submodule "nginx"]
cookbook 'nginx', 'github: chrishenderson/nginx'
#[submodule "ntp"]
cookbook 'ntp', 'github: opscode-cookbooks/ntp'
#[submodule "postgresql"]
cookbook 'postgresql', 'github: opscode-cookbooks/postgresql'
#[submodule "runit"]
cookbook 'runit', 'github: chrishenderson/runit'
#[submodule "ant"]
cookbook 'ant', 'github: opscode-cookbooks/ant'
#[submodule "apache2"]
cookbook 'apache2', 'github: opscode-cookbooks/apache2'
#[submodule "ark"]
cookbook 'ark', 'github: opscode-cookbooks/ark'
#[submodule "aws"]
cookbook 'aws', 'github: opscode-cookbooks/aws'
#[submodule "bluepill"]
cookbook 'bluepill', 'github: opscode-cookbooks/bluepill'
#[submodule "build-essential"]
cookbook 'build-essential', 'github: opscode-cookbooks/build-essential'
#[submodule "chef_handler"]
cookbook 'chef_handler', 'github: opscode-cookbooks/chef_handler'
#[submodule "database"]
cookbook 'database', 'github: opscode-cookbooks/database'
#[submodule "dmg"]
cookbook 'dmg', 'github: opscode-cookbooks/dmg'
#[submodule "git"]
cookbook 'git', 'github: chrishenderson/git'
#[submodule "logrotate"]
cookbook 'logrotate', 'github: opscode-cookbooks/logrotate'
#[submodule "memcached"]
cookbook 'memcached', 'github: chrishenderson/memcached'
#[submodule "ohai"]
cookbook 'ohai', 'github: opscode-cookbooks/ohai'
#[submodule "openssl"]
cookbook 'openssl', 'github: opscode-cookbooks/openssl'
#[submodule "perl"]
cookbook 'perl', 'github: opscode-cookbooks/perl'
#[submodule "php"]
cookbook 'php', 'github: opscode-cookbooks/php'
#[submodule "postfix"]
cookbook 'postfix', 'github: opscode-cookbooks/postfix'
#[submodule "python"]
cookbook 'python', 'github: opscode-cookbooks/python'
#[submodule "vim"]
cookbook 'vim', 'github: opscode-cookbooks/vim'
#[submodule "windows"]
cookbook 'windows', 'github: opscode-cookbooks/windows'
#[submodule "xfs"]
cookbook 'xfs', 'github: opscode-cookbooks/xfs'
#[submodule "xml"]
cookbook 'xml', 'github: opscode-cookbooks/xml'
#[submodule "yum"]
cookbook 'yum', 'github: opscode-cookbooks/yum'
#[submodule "rsyslog"]
cookbook 'rsyslog', 'github: opscode-cookbooks/rsyslog'
#[submodule "hostsfile"]
cookbook 'hostsfile', 'github: foxycoder/hostsfile'
#[submodule "rbenv"]
cookbook 'rbenv', 'github: ddux/rbenv-cookbook'
#[submodule "yumrepo"]
cookbook 'yumrepo', 'github: bryanwb/cookbook-yumrepo'
#[submodule "htpasswd"]
cookbook 'htpasswd', 'github: Youscribe/htpasswd-cookbook'
